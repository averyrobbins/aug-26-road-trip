  ...
  function initializeMap(purchases, states, stateName) {
    ...
    function select(id) {
      ...
      if (!row) {$("purchase-detail-heading").textContent = "Choose a stop"; content.append(element("p", "Select a map point or a purchase in the list to see its details."));}
        ...
        if (row.fuel_gallons !== null && row.fuel_gallons !== undefined) {fact("Fuel quantity", `${fixed(row.fuel_gallons)} gallons`); fact("Posted fuel price", `$${fixed(row.fuel_unit_price)}/gallon`); fact("Printed fuel grade", row.fuel_grade || "Not captured");}
